<?php
// assume variables already passed from controller
?>
    ...
    <div class="container-fluid px-0">
        ...
        <div class="collapse navbar-collapse justify-content-end">
            <ul class="navbar-nav align-items-center">
                <li class="nav-item">
                    <span class="user-email d-none d-lg-inline">
                        <?= htmlspecialchars($_SESSION['email_address']) ?>
                    </span>
                </li>
    ...
    <div class="card border rounded-4 p-4 p-md-5" style="max-width: 520px;">
        ...
        <div class="bg-light p-3 rounded-3 mb-4">
            ...
            <div class="d-flex justify-content-between py-2 border-bottom">
                ...
                <span class="fw-semibold">
                    <?= htmlspecialchars($teacher->teacher_type ?? 'General Lessons') ?>
                </span>
            </div>

            <div class="d-flex justify-content-between py-2 border-bottom">
                <span class="text-secondary text-uppercase small fw-semibold">Day</span>
                <span class="fw-semibold"><?= htmlspecialchars($display_day) ?></span>
            </div>

            <div class="d-flex justify-content-between py-2 border-bottom">
                <span class="text-secondary text-uppercase small fw-semibold">Date</span>
                <span class="fw-semibold"><?= htmlspecialchars($display_date) ?></span>
            </div>

            <div class="d-flex justify-content-between py-2 border-bottom">
                <span class="text-secondary text-uppercase small fw-semibold">Time</span>
                <span class="fw-semibold">
                    <?= htmlspecialchars($display_start) ?> - <?= htmlspecialchars($display_end) ?>
                </span>
            </div>
            ...
            <?php if ($hourly_rate): ?>
                <div class="d-flex justify-content-between py-2 border-bottom">
                    <span class="text-secondary text-uppercase small fw-semibold">Rate</span>
                    <span class="fw-semibold">£<?= number_format($hourly_rate, 2) ?>/hour</span>
                </div>

                <div class="d-flex justify-content-between py-2 bg-success-subtle rounded-2 mt-2 px-2">
                    <span class="fw-semibold">Total Cost</span>
                    <span class="fw-bold text-success">£<?= number_format($total_cost, 2) ?></span>
                </div>
            <?php endif; ?>
        </div>

        <div class="alert alert-warning mb-4">
            Booking this slot reserves the next available occurrence only.
        </div>
            ...
            <?php if ($canBook): ?>
                <form action="confirmBooking.php" method="POST" class="flex-grow-1">
                        ...
                        <div class="mb-2">
                            <select name="student_email" class="form-select" required>
                                ...
                                <?php foreach ($linkedStudents as $student): ?>
                                    <option value="<?= htmlspecialchars($student->email_address) ?>">
                    ...
                    <button type="submit" class="btn btn-dark w-100"
                        onclick="return confirm('Book this lesson for £<?= number_format($total_cost ?? 0, 2) ?>?')">
                        Book (£<?= number_format($total_cost ?? 0, 2) ?>)
                    </button>
